...
import { List } from 'components/publisher';
import { Filter } from 'components/publisher';
import { Modal } from 'components/publisher';
...
const Publishers: React.FC<Props> = ({ publishers }) => {
  ...
  const handleFilter = (query: any) => {
    let newList: Publisher[] = list;
    const name = query.name;
    const createTime = query.createTime;
    if (name) {
      newList = newList.filter((item) => item.name.includes(name));
    }
